<template>
  <section class="content">
    <ContentHeaderVue class="pl-0" :name="'Free roam VR'" />
    ...
    <div class="my-5">
      <button type="button"
        ...
        data-toggle="modal" data-target="#exampleModal" @click="clickModal()">Upload App</button>
    </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form @submit.prevent="save">
              ...
              <div class="form-group my-3" :class="errors.path ? 'is-valid' : ''">
                <label for="recipient-name" class="col-form-label">Path to .exe file :</label>
                <textarea type="text" class="form-control text-xl border-gray-300" :class="errors.path ? 'is-valid' : ''"
                  v-model="form.path" id="recipient-name"></textarea>
              ...
              <div v-if="form.packageName" class="form-group" :class="errors.packageName ? 'is-valid' : ''">
                ...
                <input type="text" class="form-control text-xl" :class="errors.packageName ? 'is-valid' : ''"
                  v-model="form.packageName" id="recipient-name" placeholder="com.holomia.holomia" readonly />
              ...
              <div v-if="form.packageName" class="form-group" :class="errors.version ? 'is-valid' : ''">
                ...
                <input type="text" class="form-control text-xl" :class="errors.version ? 'is-valid' : ''"
                  v-model="form.version" id="recipient-name" readonly />
                ...
                <div class="flex items-center justify-center w-full">
                  <label class="flex flex-col w-full h-48 border-4 border-dashed hover:bg-gray-100 hover:border-gray-300">
                    <div class="flex flex-col items-center justify-center pt-7">
                      ...
                      <p class="pt-1 text-sm tracking-wider text-gray-400 group-hover:text-gray-600">Drag file .jpg .jepg
                        .png</p>

                      <img :src="image" width="50px" />
                      <input type="file" @input="form.icon = $event.target.files[0]"
                        :class="errors.icon ? 'border-red-500' : ''" class="opacity-0" accept="image/*"
                        @change="onChangeFile" />
                    </div>
                  </label>
              ...
              <div class="modal-footer">
                ...
                <button @click.prevent="save()" type="submit"
                  class="inline-block px-6 py-2.5 bg-blue-600 text-white font-black text-xl leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-700 active:shadow-lg transition duration-150 ease-in-out">Upload</button>
              </div>
            </form>
          </div>
        </div>
    ...
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg mt-5">
      <table class="w-full text-xl text-left text-gray-500 ">
        ...
        <tbody>
          <tr v-for="(app, index) in window_apps.data" :key="index" class="bg-white border-b ">
            ...
            <th apk="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap ">{{ app.name }}</th>
            <th apk="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap ">{{ app.path }}</th>
            <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap ">
              <img :src="app.icon" width="50px" />
            </th>
            <th apk="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap ">{{ app.packageName }}</th>
            <th apk="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap ">{{ app.version }}</th>
            <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap "
            ...
            <td class="py-4 px-6 text-right">
              <button @click="edit(app)" type="button" data-toggle="modal" data-target="#exampleModal"
                class="inline-block px-6 py-2.5 bg-gray-200 text-gray-700 font-black text-xl leading-tight uppercase rounded shadow-md hover:bg-gray-300 hover:shadow-lg focus:bg-gray-300 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-400 active:shadow-lg transition duration-150 ease-in-out mx-2">
                Edit
                Path
              </button>
              <button type="button" @click="Delete(app.id)"
