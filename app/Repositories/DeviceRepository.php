...
class DeviceRepository extends BaseRepository
{
    ...
    public function getDeivces($relations, $request, $appends, $sortBy, $sort_Direction)
    {
        ...
        })->orderBy($sortBy, $sort_Direction)->paginate(2)->appends($appends) :  $this->model()->with($relations)->where(function ($query) use ($request) {
            $query->where('name', 'LIKE', '%' . $request->term . '%')
                ->orWhere('device_id', 'LIKE', '%' . $request->term . '%');
        })->orderBy($sortBy, $sort_Direction)->paginate(2)->appends($appends);
    }
